...
try{
  ...
  const result=await page2.evaluate(async()=>{
    ...
    const data=await import('./src/data/events.js?v=1.5.27');
    const contract=await import('./src/engine/contract.js?v=1.5.27');
    const draft=await import('./src/engine/draft.js?v=1.5.27');
    const abilities=await import('./src/data/abilities.js?v=1.5.27');
