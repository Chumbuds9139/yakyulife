...
try{
  ...
  const result=await page2.evaluate(async()=>{
    ...
    const data=await import('./src/data/events.js?v=1.5.25');
    const contract=await import('./src/engine/contract.js?v=1.5.25');
    const draft=await import('./src/engine/draft.js?v=1.5.25');
    const abilities=await import('./src/data/abilities.js?v=1.5.25');
