...
try{
  ...
  const result=await page.evaluate(async()=>{
    const state=await import('./src/core/state.js?v=1.5.17');
    const timeline=await import('./src/ui/timeline.js?v=1.5.17');
    const phases=await import('./src/flow/phases.js?v=1.5.17');
    ...
    const draft=[...document.querySelectorAll('#act button')].find(b=>b.textContent.includes('投入日本職棒選秀'));
    if(!draft)throw new Error('找不到大學季前日本職棒選秀按鈕');
    draft.click();
  ...
  assert(result.latest.stage.startsWith('日職 · '),JSON.stringify(result));
  assert(!result.latest.stage.includes('大學'));
  assert(!result.latest.lab.startsWith('大'));
  assert(result.leftText.includes('日職'));
