...
try{
  ...
  const r=await page.evaluate(async()=>{
    ...
    const season=await import('./src/engine/season.js?v=1.6.2');
    const injury=await import('./src/engine/injury.js?v=1.6.2');
    const phases=await import('./src/flow/phases.js?v=1.6.2');
    ...
    /* ⑤ 強制轉回：門檻是該層級 min − TW_BAR。
       日本版中職一軍 min=53（原作 41），過中職／不過大聯盟的窗口是 52.5～55.5。 */
    const audit={};
    ...
    let s;
    const okCPBL={sta:58,vel:55,ctl:54,brk:53,con:62,pow:60,spd:50,eye:56};
    s=mk(okCPBL); s.__fired=phases.twoWayAudit(); audit.both=snap(s);
    s=mk({sta:58,vel:55,ctl:54,brk:53,con:20,pow:20,spd:20,eye:20}); s.__fired=phases.twoWayAudit(); audit.batDead=snap(s);
    s=mk({sta:58,vel:18,ctl:18,brk:18,con:62,pow:60,spd:50,eye:56}); s.__fired=phases.twoWayAudit(); audit.pitDead=snap(s);
    s=mk({sta:58,vel:18,ctl:18,brk:18,con:62,pow:60,spd:50,eye:56},{twOrigin:'genius'});
    s.__fired=phases.twoWayAudit(); audit.organic=snap(s);
    /* 同一組能力在大聯盟門檻更嚴（min 56 → 55.5）。 */
    s=mk(okCPBL,{lv:'CPBL1'});
    s.__fired=phases.twoWayAudit(); audit.cpbl=snap(s);
    s=mk(okCPBL,{lv:'MLB',org:'MiLB'});
