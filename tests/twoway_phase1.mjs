...
try{
  ...
  const unit=await page.evaluate(async()=>{
    ...
    const ability=await import('./src/engine/ability.js?v=1.6.4');
    const events=await import('./src/flow/events.js?v=1.6.4');
    const {POS_AB,POSN}=await import('./src/data/abilities.js?v=1.6.4');
  ...
  const walk=await page2.evaluate(async()=>{
    const state=await import('./src/core/state.js?v=1.6.4');
    const {TW_SIX_GUARANTEED}=await import('./src/flow/phases.js?v=1.6.4');
