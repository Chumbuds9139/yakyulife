...
import {chance, clamp} from '../core/rng.js?v=1.6.2';
import {DPN, GLOVE_TH, GLOVE_K} from '../data/abilities.js?v=1.6.2';
import {LV} from '../data/teams.js?v=1.6.2';
import {card} from '../ui/dom.js?v=1.6.2';
import {tlNote} from '../ui/timeline.js?v=1.6.2';
import {isSP, slgOf, baseballERA, pitG} from './season.js?v=1.6.2';
import {isCareerScoringAward, splitBySide} from './award-rules.js?v=1.6.2';
...
export function awards(bucket,st){
  ...
  if(added.length){
    ...
    if(annualMvp&&S.age<24&&(bucket==='NPB'||bucket==='MLB')){
      const unlock=(key,name)=>{ if(!S.traits[key])traitCard(key,name,'天空才是你的極限，怪物的成績，不過是你傳奇生涯的起點'); };
      if(S.pos==='TW'){
        if(pitchMvpQual())unlock('strongpitch','強投少年');
        if(batMvpQual())unlock('stronghit','強打少年');
      }else{
