...
/* 聯盟強度折算：同一項能力在較弱聯盟(較低par)會換算出好看得多的數據，且該數據幾乎每季
   ...
   終結者(CL)又再獨立一組折算值：終結者的生涯總分主要靠救援數與救援里程碑堆疊，兩者都有
   明確天花板(出賽數上限、救援轉換率上限、里程碑加分頂多到500救援)，能力再往上加分也不太
   會再成長，套用跟先發一樣的折算會讓終結者在較弱聯盟幾乎沒有分數成長空間、永遠碰不到名
   人堂；套用跟打者一樣的折算又會讓普通終結者也洗得進名人堂，所以另外校準一組介於中間、
   且分數成長空間本來就窄的折算值。 */
export const LEAGUE_K={CPBL:{P:0.30,H:0.48,CL:0.37},NPB:{P:0.54,H:0.65,CL:0.54},MLB:{P:1,H:1,CL:1}};
