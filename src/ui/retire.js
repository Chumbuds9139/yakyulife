import {S, blankStat, bucketOf, CAREER_STAT_BUCKETS, CAREER_EVAL_BUCKETS} from '../core/state.js?v=1.5.27';
import {R, ri, SEED} from '../core/rng.js?v=1.5.27';
import {OFFICIAL_URL} from '../config.js?v=1.5.27';
import {LV, LG_N, CPBL_TEAMS, NPB_TEAMS, MLB_TEAMS, INDEP_TEAMS, CORPORATE_TEAMS, teamNick, npbStadium} from '../data/teams.js?v=1.5.27';
import {TIER_TH, FAN, RP_LV_SUF} from '../data/economy.js?v=1.5.27';
import {TRAIT_KEYS} from '../data/traits.js?v=1.5.27';
import {$, card, choose, divider, board, actClear} from './dom.js?v=1.5.27';
import {careerTimelineCard, tlNote} from './timeline.js?v=1.5.27';
import {traitNames, traitTagStyle, traitColorRank} from './traits.js?v=1.5.27';
import {roleN, fmtIP, slgOf, baseballERA, baseballWHIP} from '../engine/season.js?v=1.5.27';
import {fmtMoney} from '../engine/contract.js?v=1.5.27';
import {isChampionshipYear, isProChampionshipYear} from '../engine/championship.js?v=1.5.27';
import {capTeam, careerMilestones, honorGroups, posLegendPhrase, primaryPos, statTable, tierOf, yearRanges, honorText} from '../engine/career.js?v=1.5.27';
import {shareImageSheet} from './share-image.js?v=1.5.27';
/* ================= 結算圖資料建構 =================
...
那是甲子園決勝的隔天。前一晚你投完延長，監督只問一句：「還能投吧？」你點頭。隔天再先發。

痛是後來才說得出口的。當時你只覺得，那是王牌該有的重量。

你去問監督，監督說：「還會痛就是還不夠強。」於是你多練了兩百顆。
...
你去拜訪高校、大學、社會人，講了三十幾場，大部分的反應都一樣：

「甲子園以前也沒這些，還不是照樣出王牌。」

「夏季大會就是要比氣魄，弄那麼複雜。」

有一次一個資深監督當面問你：「你成績後來那麼差，是因為沒有這個嗎？」
...
因為在球場上，說痛是一件軟弱的事情、是會讓自己從此失去先發的事情。
...
他的球速很棒，高三就147公里，變化球犀利，是縣大會的當家王牌。決勝再登板的名單上，有他的名字。
...
而過沒多久，你接到電話，那個監督大罵，是你毀掉了這個投手的甲子園。
...
某天早上你在辦公室泡咖啡，電視轉播著日職的比賽。

那個高中球速147公里的投手，現在是日職一軍的先發，球速已經到了154公里。
...
某個夏天的傍晚，你在辦公室整理報告，電視上轉播著一場日職比賽。

三十四歲的外野手站上打擊區，賽季打擊率三成一。

主播說：「他生涯後期反而越打越好，真的很不簡單。」

解説接了一句：「他自己說過，是在二十六歲那年，重新學會怎麼用身體。」
...
export const SECOND_CAREER_ENDINGS=[
  ...
  `你回到母校當監督，月薪不高，但你把自己沒走完的路畫成地圖交給學弟。第七年，你帶的投手在選秀會上被第一輪指名，電視轉播帶到你的時候，你哭得比他還慘。`,
...
export function retireScene(tiers){
  ...
  if(lg==='CPBL'){
    ...
    txt=`企業隊的最後一場，觀眾席上大半是同事與廠裡的後輩。終場哨響，監督走過來拍你的肩：「今年的都市對抗，有你就夠了。」你把釘鞋收進袋子——明天還要上班。沒有胴上げ，但休息室的桌上放著一盒簽名球，寫著「お疲れさま」。`;
  }else if(lg==='INDEP'){
    txt=`獨立聯盟的觀眾不多，但他們記得你的背號。終場後有孩子隔著圍網要簽名。沒有煙火，只有一台巴士等著開往下一座小鎮。你在車窗上哈了一口氣，用手指寫下今年的最後一個數字，然後擦掉。`;
  }else{
...
export function endGame(reason){
  ...
  { const LGR={CPBL:0,NPB:1,MLB:2}, CTY={CPBL:'台灣',NPB:'日本',MLB:'美國'};
    ['CPBL','NPB','MLB'].forEach(low=>{ ['CPBL','NPB','MLB'].forEach(high=>{
      if(LGR[high]>LGR[low] && tiersByLg[low] && tiersByLg[high] && tiersByLg[low].i<=1 && tiersByLg[high].i>=3){
        picks.push(`在${CTY[low]}是${LG_N[low]}的招牌，到了${CTY[high]}的${LG_N[high]}卻完全打不出來——「這人是誰？」當地球迷一臉問號，簽他的球團補強真的有夠瞎`);
      }
    }); });
  }
  ...
  if(S.love.caught)picks.push('球打得還可以，但私生活真的有夠亂，週刊比他的比賽精華還多');
  if(S.traits.scum)picks.push('實在很不想恭喜他引退。想到他做過的事情，祝福不出口。');
  if(S.traits.franchise)picks.push(S.franchiseActive?'一隊一人，引退背號準備掛上去了。謝謝你留下來':'雖然你離開了，但還是謝謝你那段時間帶給我們球隊的希望');
  if(S.traits.legend)picks.push('這種千年一遇的球員，能夠看到他的比賽，真的是最幸福的事情');
  if(S.traits.intlace)picks.push('穿上日本代表球衣的那個男人，永遠的國家英雄');
  if(S.traits.samurai)picks.push('六度披上國家隊戰袍，從不推辭。他比劃胸口的那一幕，我手機桌布放到現在');
  if(S.traits.disc)picks.push('自律到可怕，凌晨四點的球場都認得他');
  if(S.traits.favorite)picks.push('他真的很有監督愛欸，我一度懷疑他是監督的兒子');
  if(S.traits.cancer)picks.push('真的是休息室的空氣污染源，這種人就趕快引退吧');
  if(S.traits.thief)picks.push('年俸和成績完全對不起來，打不好還是有高薪，真的有夠爽');
  if(S.traits.mrteam)picks.push('十五年只為一隊，'+(teamNick(S.mrTeamName||'')||'')+'先生這個稱號，他當之無愧');
  if(S.traits.confidante)picks.push('場上叱吒風雲，感情卻總是停在朋友。同行的淚。');
  if(S.traits.smallschool)picks.push('從那種無名高校打到職業，這故事夠拍一部電影了');
