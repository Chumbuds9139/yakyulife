...
import {APP_VER, OFFICIAL_HOST} from '../config.js?v=1.6.5';
import {TEAM_COLOR, LG_N} from '../data/teams.js?v=1.6.5';
import {RP_TICKS} from '../data/economy.js?v=1.6.5';
import {TRAIT_KEYS} from '../data/traits.js?v=1.6.5';
import {$, teamChip, modalOpen, modalClose} from './dom.js?v=1.6.5';
import {THEME_NAMES} from './prefs.js?v=1.6.5';
import {traitNames, traitColorRank} from './traits.js?v=1.6.5';
import {fmtMoney} from '../engine/contract.js?v=1.6.5';
import {twoWayView} from '../engine/career.js?v=1.6.5';
import {rpTagline, rpFamily, RP_F3, RP_F2, rpCumData, rpIntlData, rpHonorItems, rpOrgOf, rpProData, rpSalaryData, championshipYear} from './retire.js?v=1.6.5';
...
export function renderShareImage(evals,picks,opt){
  ...
  const LGC={MLB:C_INFO,NPB:C_BAD,CPBL:C_ACC,INDEP:C_DIM,CORP:C_DIM,MINOR:C_DIM};
  ...
  function tagColor(o){ /* keep in sync with traitTagStyle() + the .tag defaults */
    ...
    if(o.key==='legend'||o.key==='samurai'||o.key==='intlace'||o.key==='pitcherTC'||o.key==='hitterTC'||o.key==='nitenichi')return {bg:'#3a2c05',bd:'#ffc95c',fg:'#ffe08a'}; /* 金 */
  ...
  function render(c){
    ...
    intl.forEach(({n,d})=>{
      ...
      sec('國際賽逐屆成績（日本代表 '+S.intlCount+' 屆）'+(n?'・'+n:''));
