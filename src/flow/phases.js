import {S, stepQ, nextStep, stageLabel} from '../core/state.js?v=1.5.2-r1';
import {R, ri, chance, clamp} from '../core/rng.js?v=1.5.2-r1';
import {ABL, POS_AB} from '../data/abilities.js?v=1.5.2-r1';
import {LV, PATHS, teamNick} from '../data/teams.js?v=1.5.2-r1';
import {AMA_ANNUAL} from '../data/economy.js?v=1.5.2-r1';
import {card, choose, board, divider} from '../ui/dom.js?v=1.5.2-r1';
import {tlNote, tlPush, tlRestage} from '../ui/timeline.js?v=1.5.2-r1';
import {allocUI} from '../ui/alloc.js?v=1.5.2-r1';
import {addAb, ovr, dposReview, statBonusTxt} from '../engine/ability.js?v=1.5.2-r1';
import {rollInjury, tjCap} from '../engine/injury.js?v=1.5.2-r1';
import {isMrTeamEligible} from '../engine/tenure.js?v=1.5.2-r1';
import {amateurSeason, proSeason, slgOf, currentSalaryRating, baseballERA, baseballWHIP} from '../engine/season.js?v=1.5.2-r1';
import {championshipChance} from '../engine/championship.js?v=1.5.2-r1';
import {buyoutRemaining, contractAnnual, contractMarketProfile, controlledAnnual, crossOffers, daibaFarewell, extensionOffer, faFlow, fmtMoney, handleDemotion, levelMinAnnual, makeContract, makeOffers, offseasonTradeCheck, pickOfferUI, signTo, teamChampRate} from '../engine/contract.js?v=1.5.2-r1';
import {drawEvents, removeTrait, checkChampionTrait} from './events.js?v=1.5.2-r1';
import {loveEvent} from './love.js?v=1.5.2-r1';
import {runDraft, pathChoiceHS, pathChoiceU4, advance} from '../engine/draft.js?v=1.5.2-r1';
import {endGame} from '../ui/retire.js?v=1.5.2-r1';
/* ================= 年度流程 ================= */
...
export function phasePre(){
  ...
  if(declAge>=32){ const baseDec=declAge>=35?5+(declAge-35):2;
    ...
    const declineText=S.pos==='C'
      ?`配球以外能力 <b class="dn">降 ${dec} 級</b>（你的配球經驗是你珍貴的財產，不會急遽衰退，配球<b class="dn">只降 ${catcherCallDec} 級</b>）`
      :`所有能力 <b class="dn">降 ${dec} 級</b>`;
    card('bad','歲月不饒人',`${declAge>=35?'第二階段（逐年加劇）':'第一階段'}衰退：${declineText}${S.traits.disc?'（自律狂：生涯延後兩年）':''}${oldGhostActive?`（老鬼：原本要降 ${baseDec} 級，本年減緩 50%）`:''}。訓練加點照常，但身體回不去了。`); board(0); }
  ...
  let afterAsk=()=>{
    ...
    if(S.traits.combo && !S.skipMid && (S.comboKey||S.samePickKey)) {
      ...
      if(gained > 0) cmsg += `（加了 ${cv-overflow} 點，升了 <b class="up">${gained}</b> 級）`;
      if(overflow > 0) cmsg += `（頂峰造極：溢出的 ${overflow} 點轉為${statBonusTxt(overflow)}）`;
      if(gained===0 && overflow===0) cmsg += `（加了 ${cv} 點，尚未升級——不足一級，點數保留在進度槽）`;
      msg += cmsg + `。`;
    }
    ...
    if(S.six>=5&&!S.traits.genius&&S.age<22){ S.traits.genius=true;
      ...
      boost.forEach(k=>{ S.pot[k]=Math.min(80,(S.pot[k]||62)+10);
        S.ab[k]=clamp(S.ab[k]+5,1,80); bl.push(`${ABL[k]} <b class="up">升 5 級</b>（潛力上限提高 10 級 → ${S.pot[k]}）`); });
