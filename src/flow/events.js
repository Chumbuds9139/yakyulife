...
import {R, pick, chance, clamp} from '../core/rng.js?v=1.5.24';
import {ABL, POS_AB} from '../data/abilities.js?v=1.5.24';
import {LV} from '../data/teams.js?v=1.5.24';
import {EVENTS, EVENT_CATEGORY_NAMES, EVENT_COMBINATIONS, EVENT_ROUTES, eventInjuryRisk} from '../data/events.js?v=1.5.24';
import {card, choose, board} from '../ui/dom.js?v=1.5.24';
import {addAb, statBonus, statBonusTxt, abGainTxt, ovr} from '../engine/ability.js?v=1.5.24';
import {majorChampionshipCount} from '../engine/championship.js?v=1.5.24';
...
export function evOdds(){
  /* 彩蛋球員只由 S.invincible 驅動，不污染任何正常 trait；事件卡三種選擇皆為 100%。 */
  if(S.invincible)return {safe:100,norm:100,bold:100};
...
function eventCash(mode){
  ...
  const invincibleBonus=S.invincible?2:1;
  return Math.max(1,Math.round(base*({bold:1.5,norm:1,safe:.5}[mode]||1)*traitBonus*invincibleBonus));
}
...
export function eventPlan(category,mode,good,clutchTier){
  const soft=clutchTier>=1, full=clutchTier>=2;
...
export function resolveEvent(ev,mode,done){
  ...
  let good,tag;
  if(mode==='safe'){ good=S.invincible?true:chance(od.safe); tag='保守應對'; }
  else if(mode==='bold'){ good=S.invincible?true:chance(od.bold); tag='全力一搏';
    if(good){ S.cntBoldWin++; if(ev.category==='endorsement')S.cntEndorseBoldWin=(S.cntEndorseBoldWin||0)+1; }
    else S.cntBoldFail++; }
  else { good=S.invincible?true:chance(od.norm); tag=''; if(good)S.cntNormWin=(S.cntNormWin||0)+1; }
  ...
  const injuryRisk=S.invincible?0:eventInjuryRisk(ev,mode,good,!!S.traits.clutch);
